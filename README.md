...
| 数据库 | MySQL 8.0 |
| 缓存 | Redis 7.x + Redisson + Caffeine + Guava Cache |
...
### only-test
集成测试沙箱，包含 23 类 Demo 示例，覆盖 Spring Boot 常用组件及中间件集成。
...
| 13 | LLM 聊天 | OpenAI 兼容接口同步 + SSE 流式 |
| 14 | 幂等 | Redis SET NX + 数据库唯一键 |
| 15 | 限流 | Guava RateLimiter（本地）+ Redis 滑动窗口（分布式） |
| 16 | 异步编排 | CompletableFuture allOf / anyOf / exceptionally |
| 17 | 多数据源 | baomidou dynamic-datasource 读写分离 |
...
│       ├── config/       # MyBatis-Plus, Redis, Async, ES 配置
│       ├── controller/   # REST 接口
│       ├── service/      # 业务逻辑
│       ├── mapper/       # MyBatis-Plus Mapper
│       ├── entity/       # 数据实体
│       ├── dto/          # 传输对象
│       ├── async/        # CompletableFuture 异步编排
│       ├── datasource/   # 多数据源读写分离
│       ├── idempotent/   # 幂等
│       ├── idgen/        # 分布式 ID
│       ├── localcache/   # 本地缓存（Guava + Caffeine）
│       ├── lock/         # Redisson 分布式锁
│       ├── mq/           # RocketMQ 消息
│       ├── ratelimit/    # 限流
│       ├── retry/        # Spring Retry
